...
│   ├── train_fold1.parquet   # 10-fold 분할 데이터 (자동 생성)
│   ├── ...                   # train_fold2.parquet ~ train_fold10.parquet
...
3. **3순위**: `feat_e_3` 존재 + `clicked=0` → **9개 fold로 분할** (10번째 fold는 전체 포함)
...
1. **데이터 분류**: `feat_e_3` 기준으로 3개 그룹으로 분류
2. **즉시 저장**: 각 그룹을 별도 parquet 파일로 저장 (메모리 효율성)
3. **Fold 생성**: 각 fold별로 데이터를 조합하여 `train_foldX.parquet` 생성
4. **메모리 정리**: 각 단계마다 메모리 해제

#### 📊 예상 Fold 크기
- **Fold 1-9**: 약 19,000개 행 (15MB)
- **Fold 10**: 약 95,000개 행 (78MB) - 모든 clicked=0 데이터 포함
