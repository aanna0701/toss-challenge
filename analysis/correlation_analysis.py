#!/usr/bin/env python3
"""
Correlation Analysis between Features and Click Rate
...
- Uses ANOVA for categorical variables
"""
...
def load_data():
    """Load the training data in chunks to manage memory"""
    ...
    # For correlation analysis, we can work with a sample
    # Let's take a stratified sample to maintain click rate distribution
    sample_size = min(500000, total_rows)  # Max 500K rows for analysis
    print(f"Taking sample of {sample_size:,} rows for analysis...")
    
    # Read the full dataset but sample it
    df_sample = df_info.sample(n=sample_size, random_state=42)
...
def analyze_feature_correlation(df, feature_name):
    ...
    df[f'{feature_name}_missing'] = df[feature_name].isna().astype(int)
    ...
    if var_type == 'continuous':
        ...
        if len(click_rates) > 1:
            ...
            # Calculate click rate by category
            result['click_rates_by_category'] = click_rates.to_dict('records')
        else:
