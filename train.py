...
from data_loader import load_and_preprocess_data, create_data_loaders, create_preprocessed_data_loaders
...
def train_model_with_preprocessed_data(data_dir="data", device="cuda"):
    """전처리된 데이터를 사용하는 모델 훈련 함수"""
    
    # 전처리된 데이터 로더 생성
    train_loader, val_loader, test_loader, train_dataset, val_dataset, feature_processor = create_preprocessed_data_loaders(
        data_dir=data_dir, 
    ...
    if model_type == 'tabular_seq':
        # TabularSeq 모델은 전처리된 데이터와 호환되지 않음
        raise ValueError("❌ TabularSeq 모델은 전처리된 데이터와 호환되지 않습니다. tabular_transformer를 사용하세요.")
