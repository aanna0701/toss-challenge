...
def create_classified_parquet_files():
    ...
    if missing_exists and clicked_1_exists and clicked_0_exists:
        ...
        print(f"    ✅ 기존 파일 로드 완료:")
        print(f"      - missing: {missing_count:,}개")
        ...
        print("  🔍 feat_e_3 missing 데이터 분류 및 저장 중...")
        missing_data = all_data[all_data['feat_e_3'].isna()]
        missing_count = len(missing_data)
        
        if missing_count > 0:
            missing_data.to_parquet(missing_file, engine='pyarrow', compression='snappy', index=False)
            print(f"    ✅ missing 데이터 저장: {missing_count:,}개")
            
            memory_info = psutil.virtual_memory()
            print(f"    💾 missing 처리 후 메모리: {memory_info.percent:.1f}% ({memory_info.used/(1024**3):.1f}GB)")
        
        # 2. feat_e_3 available + clicked=1 데이터 처리
        print("  🔍 feat_e_3 available + clicked=1 데이터 분류 및 저장 중...")
        clicked_1_data = all_data[(all_data['feat_e_3'].notna()) & (all_data['clicked'] == 1)]
        clicked_1_count = len(clicked_1_data)
        ...
        print("  🔍 feat_e_3 available + clicked=0 데이터 분류 및 저장 중...")
        clicked_0_data = all_data[(all_data['feat_e_3'].notna()) & (all_data['clicked'] == 0)]
        clicked_0_count = len(clicked_0_data)
...
def create_fold_parquet_files():
    ...
    # 2. clicked=0 데이터를 9-fold로 분할
    print("🔄 clicked=0 데이터를 9-fold로 분할 중...")
    if len(clicked_0_data) > 0:
        ...
        for fold in range(1, 10):  # 1부터 9까지
            fold_size = base_size
            if fold == 9:
                fold_size += remainder  # 9번째 fold에 나머지 데이터 포함
    ...
    for fold in range(1, 10):  # 1부터 9까지
        ...
        print(f"    🔄 missing 데이터 복사 중...")
        fold_data_list.append(missing_data.copy())
        memory_info = psutil.virtual_memory()
        print(f"      💾 missing 복사 후 메모리: {memory_info.percent:.1f}% ({memory_info.used/(1024**3):.1f}GB)")
