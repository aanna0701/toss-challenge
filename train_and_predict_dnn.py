...
def train_model(train_df, val_df, num_cols, cat_cols, seq_col, norm_stats, target_col, batch_size, epochs, lr, device, log_interval, save_dir):
    ...
    for epoch in range(1, epochs+1):
        ...
        for num_x, cat_x, seqs, lens, ys in pbar:
            ...
            if global_step % log_interval == 0:
                ...
                val_loss, val_score, val_ap, val_wll = evaluate_model(model, val_loader, criterion, device)
                
                # Log to CSV with immediate flush for real-time saving
                timestamp = datetime.now().strftime('%Y-%m-%d %H:%M:%S')
                with open(log_file, 'a', newline='', buffering=1) as f:  # Line buffering
                    writer = csv.writer(f)
                    writer.writerow([timestamp, epoch, global_step, train_loss, val_loss, val_score, val_ap, val_wll])
                    f.flush()  # Force write to disk immediately
                
                # Print metrics
                print(f"\n[Step {global_step}] Train Loss: {train_loss:.4f} | Val Loss: {val_loss:.4f} | Val Score: {val_score:.6f} | Val AP: {val_ap:.6f} | Val WLL: {val_wll:.6f}")
        ...
        print(f"\n[Epoch {epoch}] Average Train Loss: {epoch_loss:.4f}")
...
print(f"제출 파일 저장 완료: {save_dir}/submission.csv")
print(f"Training log: {save_dir}/training_log.csv")
print("\n=== 학습 및 추론 완료 ===")
print(f"Results saved in: {save_dir}/")
print(f"  - submission.csv: 제출 파일")
print(f"  - training_log.csv: 학습 로그")
print(f"  - best_model.pt: 최적 모델")